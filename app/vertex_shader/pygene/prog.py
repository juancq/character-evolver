...
class FuncNode(BaseNode):
    ...
    def dump_operands(self):
        ops = [self.name]
        for child in self.children:
            child_ops = child.dump_operands()
            if type(child_ops) == list:
                ops.extend(child_ops)
            else:
                ops.append(child_ops)
        return ops
    ...
    def dump(self, level=0):
        ...
        ts = []
        for child in self.children:
            ts.append(child.dump(level+1))


        if self.name == 'pow':
            return 'pow(%s, %s)' % (ts[0], ts[1])
        elif len(self.name) > 2:
            return '%s(%s)' % (self.name, ts[0])
        else:
            return '(%s %s %s)' % (ts[0], self.name, ts[1])
...
class ProgOrganism(BaseOrganism):
    ...
    def dump_operands(self):
        return self.tree.dump_operands()

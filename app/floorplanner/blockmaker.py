import wx
import igap.gui.feedbackpanel as feedbackpanel
from igap.iga.gacommon import gaParams
...
class BlockMaker(feedbackpanel.FeedbackPanel):
    def __init__(self,parent,ID=-1,dimensions=[],description=[],pos=wx.DefaultPosition,size=(220,260)):
        ...
        if appVar.has_key('texture') and appVar['texture']:
            self.draw = self.drawWithTextures
        else:
            self.draw = self.drawWithColor
...
#-------------------------------------------#
    def dc_setup(self, dc):
        ...
        return dc, coordinates, roomsizes

#-------------------------------------------#
    def drawWithColor(self, event, def_dc = None):
        '''
            Draw the floorplan rooms using a different color for each room.
        '''
        dc, coordinates, roomsizes = self.dc_setup(def_dc)
        i = 0
...
#-------------------------------------------#
    def drawWithTextures(self, event, def_dc = None):
        '''
...
#-------------------------------------------#
    def save(self, name):
        ...
        pix = wx.EmptyBitmap(self.width, self.height)
        dc = wx.MemoryDC()
        dc.SelectObject(pix)
        painted_dc = self.draw(dc)
        painted_dc.SaveFile(name, wx.BITMAP_TYPE_PNG)

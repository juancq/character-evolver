...
import webpilblockmaker as blockmaker

import igap.app.app as app
import Image
from creativity.settings import PROJECT_PATH as PATH
...
class FloorplanIndividual(Individual):
    ...
    def isequal(self, ind2):
        #if ind1.decoded_plan == ind2.decoded_plan:
        #if ind1.roomlist == ind2.roomlist:
        total = 0
        if self.decoded_plan == ind2.decoded_plan:
            total += 1
        if self.numRoom == ind2.numRoom:
            total += 1
        if self.roomarea == ind2.roomarea:
            total += 1
        if self.roomlist == ind2.roomlist:
            total += 1
        if self.roomDesc == ind2.roomDesc:
            total += 1
        if self.roomSizes == ind2.roomSizes:
            total += 1
...
class Floorplanner(app.Application):
    def __init__(self, params, random):
        ...
        self.width = self.params['plotSizeX']
        self.height = self.params['plotSizeY']
        self.texture = self.params.get('texture', None)
...
#-------------------------------------------#
    def draw(self, subset, context):
        ...
        '''
        rooms = {}
        rooms['bath'] = Image.open('%s/igap/app/floorplanner/bath.png' % PATH)
        rooms['bedroom'] = Image.open('%s/igap/app/floorplanner/bedroom.png' % PATH)
        rooms['kitchen'] = Image.open('%s/igap/app/floorplanner/kitchen.png' % PATH)
        rooms['livingroom'] = Image.open('%s/igap/app/floorplanner/livingroom.png' % PATH)

        folder = context.get('path', '')
        panels = []
        for i in xrange(len(subset)):
            filename = '%s_%d.jpeg' % (folder, i)
            p = blockmaker.BlockMaker(self.width, self.height, self.texture, rooms,
                    dimensions=subset[i].roomSizes,
                    description=subset[i].roomDesc, name = filename)
